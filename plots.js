...
function buildTable(Decade, MalesNames, num_boys, FemaleNames, num_girls) {
  ...
  for (var i = 0; i <Decade.length; i++) {
    ...
    console.log("hi")
    trow.append("td").text(Decade[i]);
    trow.append("td").text(MaleNames[i]);
    trow.append("td").text(num_boys[i]);
    trow.append("td").text(FemaleNames[i]);
    trow.append("td").text(num_girls[i]);
  }
}
